...
- **v1.0676 (25/9/2026) — Apertura 5 veces más rápida**: `_cargaInicialNube` pide las 22 claves de la nube a la vez (`Promise.all`) en vez de una por una (medido con datos reales: 6,2 s → 1,3 s; 1,2 MB, lo más pesado `data` 633 KB). Probado: lista en 0,75 s con lecturas de 300 ms, falla de red reintenta, simulador OK.
- **v1.0675 (25/9/2026) — La app no se usa hasta bajar la nube**: antes, si Firebase tardaba más de 3 s al abrir, `init()` salteaba la bajada y el dispositivo trabajaba con datos locales viejos (un cobro podía pisar la nube). Ahora: pantalla "Conectando con la nube…" (aparece si tarda más de 1,2 s) y, si falla, "Sin conexión" con Reintentar (reintenta solo cada 8 s; si Firebase ni cargó, recarga). Nada se sube hasta terminar la bajada (`window._cargaInicialOk` en `_syncFlush`); lo encolado antes se descarta (salvo el `hist` más largo, como antes). Cada lectura inicial tiene tope de 15 s (`_getNube`). Carga en `_cargaInicialNube()`. Fotos mensuales y simulador (`_esFotoOffline`) sin cambios. Probado con nube simulada: lento, datos viejos durante la bajada, cobro nuevo después, falla de red, sin internet y simulador.
- **v1.0674 (25/9/2026) — Cierre por inactividad sin cartel**: a los 60 min sin uso la sesión se cierra sola (se sacó el `alert` de `_dispLatido`; pedido de Marcos: "que salga solo").
- **v1.0673 (21/9/2026) — Simulador de cierre explicado "en criollo"**: el motor aislado del simulador (`calcularFuturoRealEnIframe`, iframe sandbox) ahora también devuelve `moraNuevas` (cuotas que el cierre real manda a mora + cuántas cuotas ya debía cada lote); nuevo bloque `_bloqueCriolloSimulador` (+ `_extrasSimulador`) en `_renderComparativo`: cuántos lotes pagaron, cuántos pasan a mora (nuevos vs que ya estaban), lista desplegable por etapa, y aviso de los lotes que finalizaron en el ciclo (Estadísticas los suma en "Total a cobrar" y el Pre cierre no). `calcularResumenCompleto` separa la mora en pesos y USD (antes sumaba USD como pesos). Solo visualización: no cambia datos ni el cierre.
- **v1.0672 (21/9/2026) — FIX desmarcar cobros de mora una cuota a la vez + botón "Desmarcar todas"**: `desmararPagoMoraCaja` identificaba el cobro solo por `ts`; las cuotas cobradas juntas comparten el mismo `ts`, así que desmarcar una borraba TODAS de la Caja de Mora y del historial de cuotas, revertía una sola (la primera, no la elegida) y bajaba `ca` en 1. Ahora cada cobro se identifica por lote+ts+mes+número de cuota (`_mismaEntradaMora`), la lógica es el helper `_revertirCobroMoraEnEstado` (no toca `ca`; la cuota queda en el historial como impaga; espejo en `bal.agrimensores` si corresponde; `esRefi`/`esInteresMes` solo sacan su línea), y `registrarCuotaHist` guarda la fecha original en `tsOrig`. Nuevo botón **"↩ Desmarcar todas (N)"** en cada lote de Balances → Caja de Mora (`desmarcarTodasMoraCaja`, confirma con lista+total y pide clave). `_deshacerUltimoCobroMora` (sin botón hoy) también corregida. Probado en memoria con el estado real del lote ETAPA 2-57 (18 pruebas).

- **v1.0381–v1.0382 (23/7/2026) — Auditoría de sincronización y protecciones**: tras el incidente, se auditaron TODAS las funciones de guardado. Se encontraron y arreglaron 5 huecos que sólo guardaban en localStorage: **`saveHist`** (el historial — la caja negra que permitió la recuperación; ahora sube a `hist` y al cargar se conserva siempre la versión más larga), **`savePendientesAprobacion`** (adelantos/pagos del colaborador → `pendientesAprobacion`), **`fj_cierre_estados`** (→ `cierreEstados`, vía nuevo helper `saveCierreEstados`), **`fj_dist_pct`** (→ `distPct`) y **`fj_mora_desactivada`** (→ `moraDesactivada`; era un interruptor global que podía diferir entre dispositivos y hacer que se cobrara distinto según dónde se abriera la app). Todas con carga inicial + `_fbListen` + inclusión en el sync manual.
  **Protecciones nuevas**: (1) `_syncBloqueaOperacion(nombre)` impide ejecutar **cierre final** (`ejecutarCierreFinal`, `hacerCierreFinal`) y **ICC** (`aplicarICC`) si hay pendientes en `_syncQueue` o un conflicto activo — son las operaciones que tocan a todos los clientes a la vez. (2) **Backup diario automático** `backupDiarioAuto()` (se dispara 12s después de cargar Firebase, una vez por día vía `fj_ultimo_backup`): guarda snapshot completo en `backups/dia{0-6}` rotando por día de semana, nunca respalda si `data` viene vacío. `verBackups()` en consola lista los disponibles.
  **Verificado empíricamente contra Firebase** (no sólo por lectura de código): alta/edición/baja de clientes, monto, cantidad de cuotas, cuota actual, trimestre, moneda, escalonado, fecha de firma, avance de cuotas del cierre + registro en hist, modificación de monto por ICC, iccMensual, icc trimestral, cuotasMora y pagos de mora. Todo sincroniza; cola en 0 tras cada prueba.
